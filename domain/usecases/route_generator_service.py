...
def get_item_by_id(_id:str, col_name:str, client: MongoClient, field: str|None = None):
    # query data using id
    db = client["trekDB"]
...
def search_nearby_places(src_id: str, dest_id: str, tags: list[str], new_approach: bool =False):
    src = requests.get(
        "https://maps.googleapis.com/maps/api/place/details/json",
...
def transform_feedback(feedback_data, daytime, weekend, types_list):
    ...
    for t in types_list:
        is_waypoint_tag = selected_check_df["waypoint_id"].apply(lambda pid: int(t in place_dict[pid].types)).rename(t)
        is_start_tag = selected_check_df["start_id"].apply(lambda pid: int(t in place_dict[pid].types)*7).rename(t)
        is_end_tag = selected_check_df["start_id"].apply(lambda pid: int(t in place_dict[pid].types)*3).rename(t)
    ...
    for d in districts_list:
        is_waypoint_district = selected_check_df["waypoint_id"].apply(lambda pid: int(d == place_dict[pid].district)).rename(d)
        is_start_district = selected_check_df["start_id"].apply(lambda pid: int(d == place_dict[pid].district)*7).rename(d)
        is_end_district = selected_check_df["start_id"].apply(lambda pid: int(d == place_dict[pid].district)*3).rename(d)
...
def prepare_place_input(
    ...
    for d in districts_list:
        ...
        candidate_df = pd.concat(
            [
                candidate_df, 
                pd.Series(sum_district_list, name=t)
            ], 
            axis=1
        )
...
def select_place(type_dict, candidate_places: list[Place]):
    ...
    for t, n in type_dict.items():
        if i > 0:
            break
        place_choice_idx = [candidate_places.index(p) for p in candidate_places if t in p.types]
        size = n*2 if n*2 <= len(place_choice_idx) else len(place_choice_idx)
        selected_place_idx = np.random.choice(a=place_choice_idx, size=size, replace=False).tolist()
    ...
    if len(rec_id) < 20 and (len(candidate_places) >= 20):
        selected_place_idx = np.random.choice(a=range(len(candidate_places)), size=20, replace=False).tolist()
        rec_id.extend(selected_place_idx)
    return rec_id
...
def select_place_feedback(type_dict, candidate_places: list[Place], prob):
    ...
    for t, n in type_dict.items():
        ...
        norm_prob = pt/pt_sum
        size = n*2 if n*2 <= len(place_choice_idx) else len(place_choice_idx)
        selected_place_idx = np.random.choice(a=place_choice_idx, size=size, replace=False, p=norm_prob).tolist()
        rec_id.extend(selected_place_idx)
        i += 1
    np.random.shuffle(rec_id)
    # random again if number of place less than 20
...
def recommend_places_new(
    ...
    candidate_places = search_nearby_places(src_id=src_id, dest_id=dest_id, tags=selected_types.keys(), new_approach=True)
    db = client["trekDB"]
    ...
    try:
        if len(feedback_data):
            ...
            y = selected_check_df.loc[:, selected_check_df.columns == 'selected'].values.flatten().tolist()
            if y.count(y[0])==len(y):
                raise
            X = selected_check_df.loc[:, selected_check_df.columns.difference(['selected', 'start_id', 'end_id', 'waypoint_id'])].values
            #* #### train model ####
            ...
            rec_idx = select_place_feedback(selected_types, candidate_places, np.transpose(y_pred_prob)[1])
        else:
